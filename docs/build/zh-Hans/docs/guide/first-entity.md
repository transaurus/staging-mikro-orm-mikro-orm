...
## Setting up CLI[​](#setting-up-cli "Direct link to Setting up CLI")
...
> Alternatively, you can use `mikro-orm.config.js` file in the root of your project, such a file will get loaded automatically. Consult [the documentation](https://mikro-orm.io/zh-Hans/zh-Hans/docs/quick-start.md#setting-up-the-commandline-tool) for more info.
...
## First Entity[​](#first-entity "Direct link to First Entity")

info

Check out the [Defining Entities](https://mikro-orm.io/zh-Hans/zh-Hans/docs/defining-entities.md) section which provides many examples of various property types as well as different ways to define your entities.
...
### Using a custom class[​](#using-a-custom-class "Direct link to Using a custom class")
...
### Defining the primary key[​](#defining-the-primary-key "Direct link to Defining the primary key")
...
### Scalar properties[​](#scalar-properties "Direct link to Scalar properties")
...
## Initializing the ORM[​](#initializing-the-orm "Direct link to Initializing the ORM")

The last missing step is to initialize the [`MikroORM`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/MikroORM.md) to get access to the [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) and other handy tools (like the [`SchemaGenerator`](https://mikro-orm.io/zh-Hans/zh-Hans/api/sql/class/SqlSchemaGenerator.md)).
...
As opposed to the async [`MikroORM.init`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/MikroORM.md#init) method, you can prefer to use synchronous variant with the constructor: [`new MikroORM()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/MikroORM.md#constructor).
...
## Working with Entity Manager[​](#working-with-entity-manager "Direct link to Working with Entity Manager")

So now you have the access to [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md), let's talk about how it works and how you can use it.

### Persist and Flush[​](#persist-and-flush "Direct link to Persist and Flush")

There are 2 methods we should first describe to understand how persisting works in MikroORM: [`em.persist()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#persist) and [`em.flush()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#flush).

[`em.persist(entity)`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#persist) is used to mark new entities for future persisting. It will make the entity managed by the [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) and once `flush` will be called, it will be written to the database.

We use [`em.create()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#create) to create entity instances.
...
To understand `flush`, let's first define what managed entity is: An entity is managed if it's fetched from the database (via [`em.find()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#find)) or registered as new through [`em.persist()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#persist) and flushed later (only after the `flush` it becomes managed).

[`em.flush()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#flush) will go through all managed entities, compute appropriate change sets and perform according database queries. As an entity loaded from the database becomes managed automatically, you do not have to call persist on those, and flush is enough to update them.
...
Remember we said the `orm.em` is a global [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) instance? Looks like it is not a good idea to use it, in fact, it is disallowed by default. Before we get to the bottom of this message, let's quickly define two more terms we haven't touched yet - the Identity Map and Unit of Work.
...
### Unit of Work and Identity Map[​](#unit-of-work-and-identity-map "Direct link to Unit of Work and Identity Map")
...
### Change Tracking[​](#change-tracking "Direct link to Change Tracking")

The identity map has a second, more important use-case. Whenever you call [`em.flush()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#flush), the ORM will iterate over the Identity Map, and for each entity it compares the original state with the values that are currently set on the entity. If changes are detected, the object is queued for an SQL `UPDATE` operation. Only the fields that changed are part of the update query.

The following code will update your database with the changes made to the `Author` object, even if you did not call [`em.persist()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#persist):
...
### Implicit Transactions[​](#implicit-transactions "Direct link to Implicit Transactions")

The most important implication of having Unit of Work is that it allows handling transactions automatically.

When you call [`em.flush()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#flush), all computed changes are queried inside a database transaction. This means that you can control the boundaries of transactions by calling [`em.persist()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#persist) and once all your changes are ready, calling `flush()` will run them inside a transaction.
...
You can find more information about transactions in [Transactions and concurrency](https://mikro-orm.io/zh-Hans/zh-Hans/docs/transactions.md) page.

### Why is Request Context needed?[​](#why-is-request-context-needed "Direct link to Why is Request Context needed?")

Now back to the validation error about global context. With the freshly gained knowledge, we know [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) maintains a reference to all the managed entities in the Identity Map. Imagine we would use a single Identity Map throughout our application (so a single global context, global [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md)). It will be shared across all request handlers, that can run in parallel.
...
### Fork to the win\![​](#fork-to-the-win "Direct link to Fork to the win!")

So we understand the problem better now, what's the solution? The error suggests it - forking. With the `fork()` method we get a clean [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) instance, that has a fresh Unit of Work with its own context and Identity Map.
...
We forgot to create the database schema. Fortunately, we have all the tools we need at hand. You can use the [`SchemaGenerator`](https://mikro-orm.io/zh-Hans/zh-Hans/api/sql/class/SqlSchemaGenerator.md) provided by MikroORM to create the schema, as well as to keep it in sync when you change your entities. For the initial testing, let's use the `refresh()` method, which is handy for testing - it will first drop the schema if it already exists and create it from scratch based on entity definition (metadata).
...
You can see the insert query being wrapped inside a transaction. That is another effect of the Unit of Work. The [`em.flush()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#flush) call will perform all the queries inside a transaction. If something fails, the whole transaction will be rolled back.

### Fetching Entities[​](#fetching-entities "Direct link to Fetching Entities")
...
Next, let's try to do the same, but with an [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) fork:
...
### Refreshing loaded entities[​](#refreshing-loaded-entities "Direct link to Refreshing loaded entities")
...
> [`FindOptions`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/interface/FindOptions.md) is the last parameter of `em.find/findOne` methods.

1. fork first, to have a clear context
2. use `disableIdentityMap: true` in the `FindOptions`
3. use `em.refresh(entity)`

The first two have pretty much the same effect, using `disableIdentityMap` just does the forking for us behind the scenes. Let's talk about the last one - refreshing. With `em.refresh()`, the [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) will ignore the contents of the Identity Map and always fetch the entity from the database.
...
We touched on creating, reading and updating entities, the last piece of the puzzle to the CRUD riddle is the delete operation. To delete entities via [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md), we have two possibilities:

1. Mark entity instance via `em.remove()` - this means we first need to have the entity instance. But don't worry, you can get one even without loading it from the database - via [`em.getReference()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#getReference).
...
So what does the [`em.getReference()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#getReference) method mentioned above do and what is an *entity reference* in the first place?
...
### Using Decorators[​](#using-decorators "Direct link to Using Decorators")
...
When using decorators, you'll need to choose a metadata provider to handle type inference. See the [Using Decorators](https://mikro-orm.io/zh-Hans/zh-Hans/docs/using-decorators.md) guide for a comprehensive overview of:
...
### Folder-based Discovery[​](#folder-based-discovery "Direct link to Folder-based Discovery")
...
This is particularly useful for large projects with many entities. See [Folder-based Discovery](https://mikro-orm.io/zh-Hans/zh-Hans/docs/folder-based-discovery.md) for details.

ESM and TypeScript file resolution

When using folder-based discovery in an ESM project with test runners like Vitest, you may encounter an error like `TypeError: Unknown file extension ".ts"` (ERR\_UNKNOWN\_FILE\_EXTENSION). This happens because the dynamic import of your entities fails to resolve TypeScript files - MikroORM performs these imports internally, and tools like Vitest cannot automatically transform them.
...
Check the [Defining Entities](https://mikro-orm.io/zh-Hans/zh-Hans/docs/defining-entities.md) documentation for more examples of all entity definition approaches.

### Working with an Existing Database[​](#working-with-an-existing-database "Direct link to Working with an Existing Database")

If you already have a database with tables and want to generate entity files from it, you can use the [Entity Generator](https://mikro-orm.io/zh-Hans/zh-Hans/docs/entity-generator.md). Install the `@mikro-orm/entity-generator` package and register the `EntityGenerator` extension:
...
Alternatively, if you prefer to keep the database schema as the source of truth and regenerate entity files on every schema change, check out the [Schema First Guide](https://mikro-orm.io/zh-Hans/zh-Hans/docs/schema-first-guide.md).

## ⛳ Checkpoint 1[​](#-checkpoint-1 "Direct link to ⛳ Checkpoint 1")

Currently, our app consists of a single `User` entity and a `server.ts` file where we tested how to work with it using [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md). You can find working StackBlitz for the current state here:
