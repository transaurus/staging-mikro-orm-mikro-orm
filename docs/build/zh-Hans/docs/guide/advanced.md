...
## Improving route registration[​](#improving-route-registration "Direct link to Improving route registration")
...
## Sign-up route[​](#sign-up-route "Direct link to Sign-up route")
...
## Custom repositories[​](#custom-repositories "Direct link to Custom repositories")
...
### Testing the `User` endpoints[​](#testing-the-user-endpoints "Direct link to testing-the-user-endpoints")
...
### JSON Web Tokens[​](#json-web-tokens "Direct link to JSON Web Tokens")
...
## Validation via Zod[​](#validation-via-zod "Direct link to Validation via Zod")

One more thing in the user module: process this new `User.social` property in the `sign-up` endpoint. Since [`em.create()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#create) is already being used, you can simply pass the social property:
...
This example only shows a very basic validation with Zod, which mirrors what MikroORM already handles - it will validate required properties and their types automatically. Check the [Property Validation](https://mikro-orm.io/zh-Hans/zh-Hans/docs/property-validation.md) section for more details.

## Rest of the Article endpoints[​](#rest-of-the-article-endpoints "Direct link to Rest of the Article endpoints")
...
### Updating entities[​](#updating-entities "Direct link to Updating entities")
...
### Upserting entities[​](#upserting-entities "Direct link to Upserting entities")
...
Read more about upserting in [Entity Manager](https://mikro-orm.io/zh-Hans/zh-Hans/docs/entity-manager.md#upsert) section.
...
While there is no such use case in this guide, a huge benefit of using the [`EntityManager`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md) with Unit of Work approach is automatic batching - all the `INSERT`, `UPDATE` and `DELETE` queries will be batched automatically into a single query per entity.
...
## Disabling change tracking[​](#disabling-change-tracking "Direct link to Disabling change tracking")
...
Let's now improve the first article endpoint - `em.findAndCount()` was used to get paginated results easily, but what if you want to customize the response? One way is with [Virtual entities](https://mikro-orm.io/zh-Hans/zh-Hans/docs/virtual-entities.md). They don't represent any database table, instead, they dynamically resolve to an SQL query, allowing you to map any kind of results onto an entity.
...
## Using QueryBuilder[​](#using-querybuilder "Direct link to Using QueryBuilder")
...
> You can find more details in the [Using Query Builder](https://mikro-orm.io/zh-Hans/zh-Hans/docs/query-builder.md) section.
...
Note how we used the `sql` helper function as a tagged template when adding the `group_concat` expression to the select clause. Read more about the support for [raw queries here](https://mikro-orm.io/zh-Hans/zh-Hans/docs/raw-queries.md).
...
## Updating the tests[​](#updating-the-tests "Direct link to Updating the tests")
...
MikroORM has a simple [result caching](https://mikro-orm.io/zh-Hans/zh-Hans/docs/caching.md) mechanism, all you need to do is add `cache` option to your [`em.find()`](https://mikro-orm.io/zh-Hans/zh-Hans/api/core/class/EntityManager.md#find) options. The value can be one of:
...
Now when you enable [debug mode](https://mikro-orm.io/zh-Hans/zh-Hans/docs/logging.md) and try to access the endpoint several times within 5 seconds, you should see just the first request producing queries.

## View entities[​](#view-entities "Direct link to View entities")

The virtual entity we created earlier evaluates its SQL expression as a subquery every time you query it. If you'd prefer to have an actual database view instead, you can convert it to a [view entity](https://mikro-orm.io/zh-Hans/zh-Hans/docs/view-entities.md) by adding the `view: true` option. The `expression` then becomes the view definition — the database will create the view once, and queries will read from it directly.
...
## Soft delete via `onFlush` event[​](#soft-delete-via-onflush-event "Direct link to soft-delete-via-onflush-event")

Let's add soft delete support for comments. Instead of physically deleting a comment from the database, we'll set a `deletedAt` timestamp and use a [filter](https://mikro-orm.io/zh-Hans/zh-Hans/docs/filters.md) to automatically exclude soft-deleted records from queries.
...
> The subscriber already checks for the `deletedAt` property generically - you can add the same `filters` option and `deletedAt` property to any entity that should support soft delete. See the [Events](https://mikro-orm.io/zh-Hans/zh-Hans/docs/events.md) and [Filters](https://mikro-orm.io/zh-Hans/zh-Hans/docs/filters.md) documentation for more details.

## Standalone scripts and CRON jobs[​](#standalone-scripts-and-cron-jobs "Direct link to Standalone scripts and CRON jobs")
...
### Basic TypeScript compilation[​](#basic-typescript-compilation "Direct link to Basic TypeScript compilation")
...
### Bundling with Vite[​](#bundling-with-vite "Direct link to Bundling with Vite")
