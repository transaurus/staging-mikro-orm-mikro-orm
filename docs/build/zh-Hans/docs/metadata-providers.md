...
For a comprehensive guide on using decorators with metadata providers, including the differences between legacy and ES spec decorators, see the [Using Decorators guide](https://mikro-orm.io/zh-Hans/zh-Hans/docs/using-decorators.md).
...
> Using `TsMorphMetadataProvider` has implications for deployment - see the [Deployment guide](https://mikro-orm.io/zh-Hans/zh-Hans/docs/deployment.md) for details on deploying applications that use this provider.
...
After the discovery process ends, all [metadata will be cached](https://mikro-orm.io/zh-Hans/zh-Hans/docs/metadata-cache.md). By default, `FileCacheAdapter` will be used to store the cache inside `./temp` folder in JSON files.
...
Alternatively, you can use [`defineEntity`](https://mikro-orm.io/zh-Hans/zh-Hans/docs/define-entity.md) instead, which is suitable for JavaScript only projects.

> If you want to have absolute control, you can also use `EntitySchema.fromMetadata()` factory method.

Read more about it in [Usage with JavaScript section](https://mikro-orm.io/zh-Hans/zh-Hans/docs/usage-with-js.md).
