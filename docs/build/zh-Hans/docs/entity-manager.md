...
The concept can be combined with the so-called `Reference` wrapper for added type safety as described in the [Type-safe Relations section](https://mikro-orm.io/zh-Hans/zh-Hans/docs/type-safe-relations.md).
...
As you can see in the fifth example, you can also use operators like `$and`, `$or`, `$gte`, `$gt`, `$lte`, `$lt`, `$in`, `$nin`, `$eq`, `$ne`, `$like`, `$re` and `$fulltext`. More about that can be found in [Query Conditions](https://mikro-orm.io/zh-Hans/zh-Hans/docs/query-conditions.md) section.
...
To stream raw results, use `QueryBuilder.stream()` or `driver.stream()` methods directly. Read more about this in the [Streaming guide](https://mikro-orm.io/zh-Hans/zh-Hans/docs/streaming.md).
