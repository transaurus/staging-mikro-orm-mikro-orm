...
MongoDB driver supports indexes and unique constraints. You can use `@Index()` and `@Unique()` as described in [Defining Entities section](https://mikro-orm.io/zh-Hans/zh-Hans/docs/defining-entities.md#indexes). To automatically create new indexes when initializing the ORM, you need to enable `ensureIndexes` option.
...
### `maxTimeMS` and `allowDiskUse`[​](#maxtimems-and-allowdiskuse "Direct link to maxtimems-and-allowdiskuse")
