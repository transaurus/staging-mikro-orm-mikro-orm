...
The `forRoot()` method accepts the same configuration object as `init()` from the MikroORM package. Check [this page](https://mikro-orm.io/zh-Hans/zh-Hans/docs/configuration.md) for the complete configuration documentation.

Alternatively you can [configure the CLI](https://mikro-orm.io/zh-Hans/zh-Hans/docs/quick-start.md#setting-up-the-commandline-tool) by creating a configuration file `mikro-orm.config.ts` and then call the `forRoot()` without any arguments. This won't work when you use a build tools that use tree shaking.
...
MikroORM supports the repository design pattern. For every entity you can create a repository. Read the complete [documentation on repositories here](https://mikro-orm.io/zh-Hans/zh-Hans/docs/repositories.md). To define which repositories shall be registered in the current scope you can use the `forFeature()` method. For example, in this way:
...
As mentioned in the [docs](https://mikro-orm.io/zh-Hans/zh-Hans/docs/identity-map.md), you need a clean state for each request. That is handled automatically thanks to the `RequestContext` helper registered via middleware.
...
MikroORM supports [dataloaders](https://mikro-orm.io/zh-Hans/zh-Hans/docs/dataloaders.md) natively since v6 for `Reference.load()` and `Collection.load()` methods.
