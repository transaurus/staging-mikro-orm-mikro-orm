...
For SQLite-specific setup (extensions, Turso, D1, custom drivers), see the dedicated [SQLite guide](https://mikro-orm.io/zh-Hans/zh-Hans/docs/usage-with-sqlite.md).
...
If you want to use database that is not currently supported, you can implement your own driver. More information about how to create one can be [found here](https://mikro-orm.io/zh-Hans/zh-Hans/docs/custom-driver.md). Then provide the driver class via `driver` configuration option:
...
Currently, you will need to maintain the database schema yourself. For initial dump, you can use [`SchemaGenerator` helper](https://mikro-orm.io/zh-Hans/zh-Hans/docs/schema-generator.md).
...
When you call `em.flush()`, all computed changes are queried [inside a database transaction](https://mikro-orm.io/zh-Hans/zh-Hans/docs/unit-of-work.md) by default, so you do not have to handle transactions manually.
...
## Oracle limitations[​](#oracle-limitations "Direct link to Oracle limitations")
