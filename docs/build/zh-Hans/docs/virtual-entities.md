...
> If you need actual database views that are created and managed by the schema generator, see [View Entities](https://mikro-orm.io/zh-Hans/zh-Hans/docs/view-entities.md) instead. View entities create `CREATE VIEW` statements and are tracked in migrations, while virtual entities evaluate their expression at query time.
