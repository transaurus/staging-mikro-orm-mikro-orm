...
## MikroORM Configuration[​](#mikroorm-configuration "Direct link to MikroORM Configuration")
...
## Service Provider[​](#service-provider "Direct link to Service Provider")
...
providers/mikro\_orm\_provider.ts
...
## RequestContext Middleware[​](#requestcontext-middleware "Direct link to RequestContext Middleware")

The most critical part of the integration is the [RequestContext](https://mikro-orm.io/zh-Hans/zh-Hans/docs/identity-map.md#request-context) middleware. It ensures each HTTP request gets its own identity map, preventing entity state from leaking between requests:

app/middleware/mikro\_orm\_middleware.ts
...
## Entity Definitions[​](#entity-definitions "Direct link to Entity Definitions")

Define entities using `defineEntity` for full type inference:

app/entities/base.ts
...
### Adding Methods and Hooks[​](#adding-methods-and-hooks "Direct link to Adding Methods and Hooks")
...
## Using in Controllers[​](#using-in-controllers "Direct link to Using in Controllers")
...
app/controllers/articles\_controller.ts
...
### User Provider[​](#user-provider "Direct link to User Provider")

Implement [`SessionUserProviderContract`](https://v6-docs.adonisjs.com/guides/database/introduction#authentication) to bridge MikroORM with the AdonisJS session guard. The `findById` method uses the `EntityManager`, which resolves to the request-scoped fork via [RequestContext](https://mikro-orm.io/zh-Hans/zh-Hans/docs/identity-map.md#request-context):

app/auth/mikro\_orm\_user\_provider.ts
...
### Auth Configuration[​](#auth-configuration "Direct link to Auth Configuration")
...
### Middleware Stack[​](#middleware-stack "Direct link to Middleware Stack")
...
app/middleware/auth\_middleware.ts
...
### Using Auth in Controllers[​](#using-auth-in-controllers "Direct link to Using Auth in Controllers")

Use `ctx.auth` to log users in and access the authenticated user:

app/controllers/users\_controller.ts
