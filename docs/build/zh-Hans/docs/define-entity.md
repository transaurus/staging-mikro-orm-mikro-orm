...
### The `defineEntity + class` pattern (recommended)[​](#the-defineentity--class-pattern-recommended "Direct link to the-defineentity--class-pattern-recommended")
...
### Pure `defineEntity` (without a class)[​](#pure-defineentity-without-a-class "Direct link to pure-defineentity-without-a-class")
...
### Reusing base properties via composition[​](#reusing-base-properties-via-composition "Direct link to Reusing base properties via composition")
...
`defineEntity.properties` (aliased as `p`) provides all [MikroORM built-in types](https://mikro-orm.io/zh-Hans/zh-Hans/docs/custom-types.md#types-provided-by-mikroorm). To use [custom types](https://mikro-orm.io/zh-Hans/zh-Hans/docs/custom-types.md), use `p.type()`:
...
Both approaches accept functions (arrow functions, named functions, async functions). The entity instance is available via `args.entity`. See [Events and Lifecycle Hooks](https://mikro-orm.io/zh-Hans/zh-Hans/docs/events.md) for the full list of available hooks and `EventArgs` details.
...
## `EntitySchema` (low-level API)[​](#entityschema-low-level-api "Direct link to entityschema-low-level-api")
...
### Using a class with `EntitySchema`[​](#using-a-class-with-entityschema "Direct link to using-a-class-with-entityschema")
