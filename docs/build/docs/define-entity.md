...
`defineEntity.properties` (aliased as `p`) provides all [MikroORM built-in types](https://mikro-orm.io/docs/custom-types.md#types-provided-by-mikroorm). To use [custom types](https://mikro-orm.io/docs/custom-types.md), use `p.type()`:
...
Both approaches accept functions (arrow functions, named functions, async functions). The entity instance is available via `args.entity`. See [Events and Lifecycle Hooks](https://mikro-orm.io/docs/events.md) for the full list of available hooks and `EventArgs` details.
